...
class BountyFeverSupabaseUser extends BaseAuthUser {
  BountyFeverSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> bountyFeverSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BountyFeverSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
